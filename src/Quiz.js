...
class Quiz extends Component {
  ...
  renderButton() {
    if (this.state.index < this.state.questions.length - 1) {
      return (
        <React.Fragment>
          ...
          <button
            ...
            onClick={(event) => this.lastQuestion()}
          >
            Back
          </button>
        </React.Fragment>
      ...
      return (
        <React.Fragment>
          <button onClick={(event) => this.finish()} className="next-button">
            Finish
          </button>
          <button
            className="back-button"
            ...
            <div className="answers-button">
              {currentQuestion.answers.map((q, i) => (
                <button
                  ...
                  id={"answer" + i}
                  className={`${
                    q === currentQuestion.correct_answer ? correct : incorrect
                  } answer-button__default animated slideInRight`}
