    ...
    state = {
      ...
      renderButton() {
        if (this.state.index < this.state.questions.length - 1) {
            return (
              <React.Fragment>
                <button className="next-button" onClick={ event => this.nextQuestion()} >Next</button>
                <button className="back-button" onClick={ event => this.lastQuestion()} >Back</button>
              </React.Fragment>
            )
        }else {
          return (
            <React.Fragment>
              <button onClick={ event => this.finish()} className="next-button">Finish</button>
              <button className="back-button" onClick={ event => this.lastQuestion()} >Back</button>
            </React.Fragment>
          )
        }
